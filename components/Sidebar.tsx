      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="flex flex-col px-4 pb-4">
          ...
          <div className="mt-4 pt-4 border-t border-slate-200">
            <h3 className="px-2 pb-2 text-xs font-semibold text-neutral-medium-gray uppercase tracking-wider">검색 결과</h3>
                ...
                filteredGrants.map(grant => (
                  <Link to={`/grant/${grant.id}`} key={grant.id}>
                    <div 
                      ...
                      `}
                    >
                      <h3 className={`font-semibold text-sm line-clamp-2 ${grantId === grant.id ? 'text-corporate-blue' : 'text-neutral-dark-gray group-hover:text-corporate-blue'}`}>
                        {grant.title}
                      </h3>
                      <div className="flex items-center justify-between mt-1">
                         <p className="text-xs text-neutral-medium-gray">#{grant.id}</p>
                         {grant.status === 'Open' ? (
